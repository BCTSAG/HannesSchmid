...
// Start test block
describe('HSMickJagger', function () {
  before(async function () {
    this.HSMickJagger = await ethers.getContractFactory('HSMickJagger');
  });

  beforeEach(async function () {
    // deploy the contract
    this.hsMickJagger = await this.HSMickJagger.deploy();
    await this.hsMickJagger.deployed();
    ...
    this.collectorContract = this.hsMickJagger.connect(signers[1]);
    ...
    for (let i = 1; i <= this.initialMintCount; i++) { // tokenId to start at 1
      await this.hsMickJagger.safeMint(this.contractOwner, "ipfs://bafkreifa22jqyp74ufyqlfp4belsljlmrframo7p3oghlmalima6qfpnfa");
      this.initialMint.push(i.toString());
    }
    console.log(this.initialMint);
  });

  // Test cases
  it('Creates a token collection with a name', async function () {
    expect(await this.hsMickJagger.name()).to.exist;
  });

  it('Creates a token collection with a symbol', async function () {
    expect(await this.hsMickJagger.symbol()).to.exist;
  });


  it('Is able to query the NFT balances of an address', async function () {
    expect(await this.hsMickJagger.balanceOf(this.contractOwner)).to.equal(this.initialMint.length);
  });

  it('Emits a transfer event for newly minted NFTs', async function () {
    let tokenId = (this.initialMint.length + 1).toString();
    await expect(this.hsMickJagger.safeMint(this.contractOwner, "ipfs://"))
      .to.emit(this.hsMickJagger, "Transfer")
  ...
  it('Is able to transfer NFTs to another wallet when called by owner', async function () {
    ...
    await this.hsMickJagger["safeTransferFrom(address,address,uint256)"](this.contractOwner, this.collector, tokenId);
    expect(await this.hsMickJagger.ownerOf(tokenId)).to.equal(this.collector);
  });

  it('Emits a Transfer event when transferring a NFT', async function () {
    let tokenId = this.initialMint[0].toString();
    await expect(this.hsMickJagger["safeTransferFrom(address,address,uint256)"](this.contractOwner, this.collector, tokenId))
      .to.emit(this.hsMickJagger, "Transfer")
  ...
  it('Approves an operator wallet to spend owner NFT', async function () {
    let tokenId = this.initialMint[0].toString();
    await this.hsMickJagger.approve(this.collector, tokenId);
    expect(await this.hsMickJagger.getApproved(tokenId)).to.equal(this.collector);
  });

  it('Emits an Approval event when an operator is approved to spend a NFT', async function () {
    let tokenId = this.initialMint[0].toString();
    await expect(this.hsMickJagger.approve(this.collector, tokenId))
      .to.emit(this.hsMickJagger, "Approval")
      .withArgs(this.contractOwner, this.collector, tokenId);
  });
  ...
  it('Approves an operator to spend all of an owner\'s NFTs', async function () {
    await this.hsMickJagger.setApprovalForAll(this.collector, true);
    expect(await this.hsMickJagger.isApprovedForAll(this.contractOwner, this.collector)).to.equal(true);
  });

  it('Emits an ApprovalForAll event when an operator is approved to spend all NFTs', async function () {
    let isApproved = true
    await expect(this.hsMickJagger.setApprovalForAll(this.collector, isApproved))
      .to.emit(this.hsMickJagger, "ApprovalForAll")
  ...
  it('Removes an operator from spending all of owner\'s NFTs', async function () {
    ...
    await this.hsMickJagger.setApprovalForAll(this.collector, false);
    expect(await this.hsMickJagger.isApprovedForAll(this.contractOwner, this.collector)).to.equal(false);
  });
  ...
  it('Allows operator to transfer all NFTs on behalf of owner', async function () {
    ...
    expect(await this.hsMickJagger.balanceOf(this.collector)).to.equal(this.initialMint.length.toString());
  });

  it('Only allows contractOwner to mint NFTs', async function () {
    await expect(this.collectorContract.safeMint(this.collector, "100")).to.be.reverted;
  });

  it('Prevents more than 150 nfts', async function () {
    for (let i = 1; i <= 147; i++) { 
      await this.hsMickJagger.safeMint(this.contractOwner, "ipfs://bafkreifa22jqyp74ufyqlfp4belsljlmrframo7p3oghlmalima6qfpnfa");
    }
    let total_supply = await this.hsMickJagger.totalSupply();
    expect(total_supply, 150);
  })
